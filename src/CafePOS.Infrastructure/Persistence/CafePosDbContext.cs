...
public sealed class CafePosDbContext(DbContextOptions<CafePosDbContext> options) : DbContext(options)
{
 ...
 protected override void OnModelCreating(ModelBuilder b)
 {
  ...
  b.Entity<CashSession>().Ignore(x=>x.ExpectedBalance); b.Entity<CashSession>().Property(x=>x.OpeningAmount).HasConversion(v=>decimal.ToInt64(decimal.Round(v*100m,MidpointRounding.AwayFromZero)),v=>v/100m); b.Entity<CashSession>().Property(x=>x.CountedAmount).HasConversion(v=>v.HasValue?decimal.ToInt64(decimal.Round(v.Value*100m,MidpointRounding.AwayFromZero)):(long?)null,v=>v.HasValue?v.Value/100m:(decimal?)null); b.Entity<CashSession>().Property(x=>x.ExpectedAtClose).HasConversion(v=>v.HasValue?decimal.ToInt64(decimal.Round(v.Value*100m,MidpointRounding.AwayFromZero)):(long?)null,v=>v.HasValue?v.Value/100m:(decimal?)null);
