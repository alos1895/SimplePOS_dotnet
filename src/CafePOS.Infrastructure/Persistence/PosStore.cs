...
public sealed class PosStore(IDbContextFactory<CafePosDbContext> factory) : IPosStore
{
 ...
 public async Task<IReadOnlyList<Order>> GetOrdersAsync(DateTimeOffset from,DateTimeOffset to,string? search,CancellationToken ct=default) { await using var db=await factory.CreateDbContextAsync(ct); var orders=await db.Orders.AsNoTracking().Include(x=>x.Items).Include(x=>x.Payments).Include(x=>x.Employee).ToListAsync(ct); var q=orders.Where(x=>x.CreatedAt>=from&&x.CreatedAt<to); if(!string.IsNullOrWhiteSpace(search)) q=q.Where(x=>x.DailyNumber.ToString().Contains(search)||x.Items.Any(i=>i.ProductName.Contains(search))); return q.OrderByDescending(x=>x.CreatedAt).ToList(); }
 ...
 public async Task<Order> SaveOrderAsync(Order order,CancellationToken ct=default)
 {
  ...
  if(!exists) { var start=new DateTimeOffset(order.CreatedAt.Year,order.CreatedAt.Month,order.CreatedAt.Day,0,0,0,order.CreatedAt.Offset); var end=start.AddDays(1); var orders=await db.Orders.AsNoTracking().Select(x=>new{x.CreatedAt,x.DailyNumber}).ToListAsync(ct); order.DailyNumber=(orders.Where(x=>x.CreatedAt>=start&&x.CreatedAt<end).Select(x=>(int?)x.DailyNumber).Max()??0)+1; db.Orders.Add(order); }
